...
import { formatBlogDate, getLocalizedBlogPosts } from '../services/blogPosts';
...
const BlogPage: React.FC<BlogPageProps> = ({ onGoBack, onSelectPost }) => {
  ...
  const posts = useMemo(() => getLocalizedBlogPosts(t), [t]);
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col gap-8 animate-fade-in">
      <div className="bg-[var(--panel-bg)] backdrop-blur-lg border border-[var(--panel-border)] rounded-xl shadow-lg p-6 md:p-10">
        <div className="flex justify-between items-center border-b border-[var(--panel-border)] pb-4 mb-8">
          <h1 className="text-3xl font-bold text-[var(--accent-color)]">{t('blog.title', 'Insights')}</h1>
          <button
            onClick={onGoBack}
            className="text-sm font-medium text-[var(--text-muted)] hover:text-[var(--text-color)] transition-colors"
          >
            &larr; {t('blog.backToLab', 'Back to the lab')}
          </button>
        </div>
        <div className="space-y-6">
          {posts.map((post) => (
            ...
            >
              ...
              <p className="text-[var(--text-muted)] text-sm">{t('blog.readMore', 'Click to read more →')}</p>
            </div>
          ))}
        </div>
        <SupportCTA
          headline={t('blog.ctaHeadline', 'Got story ideas or feedback on our hot takes?')}
          description={t('blog.ctaDescription', "Share what you'd like to see on the blog or send over your own handwriting hacks.")}
        />
      </div>
    </div>
  );
